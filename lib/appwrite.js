    ...
    GoogleSignin.configure({
      webClientId: '728223927858-bi92vr4rf79om6lvvv2slaon2ibtq2cc.apps.googleusercontent.com', // The client ID for the OAuth Consent Screen (can also work for Android)
    });
    ...
    export async function signInWithGoogle() {
      try {
        // Trigger Google Sign-In and get the ID token
        await GoogleSignin.hasPlayServices();
        const userInfo = await GoogleSignin.signIn();
        const idToken = userInfo.idToken;
    
        // Use Appwrite to create a session with the ID token
...
export async function signOut() {
  try {
    ...
    await AsyncStorage.removeItem("authToken"); // Clear token from AsyncStorage
